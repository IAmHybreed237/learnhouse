...
class TestLegacyGraceWindowConfig:
    """The window is operator-tunable; a bad value must not shorten it to zero."""

    def test_defaults_to_the_refresh_token_lifetime(self, monkeypatch):
        from src.security import session_context

        monkeypatch.delenv("LEARNHOUSE_AUTH_LEGACY_SESSION_GRACE_DAYS", raising=False)
        assert session_context._legacy_session_grace().days == 30

    def test_reads_an_operator_override(self, monkeypatch):
        from src.security import session_context

        monkeypatch.setenv("LEARNHOUSE_AUTH_LEGACY_SESSION_GRACE_DAYS", "7")
        assert session_context._legacy_session_grace().days == 7

    def test_a_negative_override_clamps_to_zero_rather_than_inverting(self, monkeypatch):
        from src.security import session_context

        monkeypatch.setenv("LEARNHOUSE_AUTH_LEGACY_SESSION_GRACE_DAYS", "-5")
        assert session_context._legacy_session_grace().days == 0

    def test_an_unparseable_override_falls_back_to_the_default(self, monkeypatch):
        from src.security import session_context

        # A typo'd value must not silently collapse the window and lock members
        # out; fall back to the default instead.
        monkeypatch.setenv("LEARNHOUSE_AUTH_LEGACY_SESSION_GRACE_DAYS", "thirty")
        assert session_context._legacy_session_grace().days == 30
